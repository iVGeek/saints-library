...
function isRelevantResult(name, title, description) {
  ...
  // Check if saint's name appears in title or description
...
async function searchAndGetBest(name, aliases = []) {
  ...
  const queries = [
    `"${name}" saint`,
    `"${name}" catholic saint`,
    name.replace(/^blessed\s+/i, '').replace(/^venerable\s+/i, '').trim() + ' saint',
  ];
  
  // Add aliases
  for (const alias of aliases.slice(0, 2)) {
    queries.push(`"${alias}" saint`);
  }
...
async function main() {
  ...
  // Find files without images - prioritize Saints over Blesseds/Venerables
  const missingFiles = [];
  ...
  for (const { file, name, aliases } of toProcess) {
    ...
    if (result) {
      ...
      console.log(`  ✓ Found: ${result.url} (via "${result.query}")`);
      updated++;
    } else {
