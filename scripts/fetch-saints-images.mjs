...
async function searchWikipedia(name) {
  // Search Wikipedia for the saint's page
...
const blockedKeywords = ['olympics', 'einstein', 'novembro', 'universal', 'almanaque', 'musicos', 'patnubay', 'agiologio', 'pronouncing', 'dictionary', 'biography', 'mythology', '1919', '1862', '1884', '2004', '2010', 'climate', 'law', 'referral', 'löv', 'isabella', 'summer', 'competition', 'roque', 'geograph', 'geograph.org.uk'];
...
async function main() {
  ...
  for (let batchStart = 0; batchStart < missingFiles.length; batchStart += BATCH_SIZE) {
    const batch = missingFiles.slice(batchStart, batchStart + BATCH_SIZE);
    let updated = 0;
    let notFound = 0;
    
    console.log(`\n=== Batch ${Math.floor(batchStart/BATCH_SIZE) + 1} (${batch.length} saints) ===`);
    
    for (const { file, name, aliases } of batch) {
      console.log(`[${totalUpdated + totalNotFound + 1}/${missingFiles.length}] Searching: ${name}`);
    ...
    fs.writeFileSync('C:\\iVGeek\\communion-of-saints\\scripts\\fetch-progress.json', 
      JSON.stringify({ totalUpdated, totalNotFound, processed: batchStart + batch.length }, null, 2));
  }
