...
async function main() {
  ...
  // Get all with images, prioritize Saints
  const withImages = [];
  for (const file of files) {
    const content = fs.readFileSync(path.join(SAINTS_DIR, file), 'utf8');
    const fm = parseFrontmatter(content);
    if (fm && fm.image) {
      withImages.push({ file, name: fm.name, image: fm.image, status: fm.canonizationStatus });
    }
  }
  
  // Sort: Saints first, then Blesseds, then Venerables
  withImages.sort((a, b) => {
    const order = { Saint: 0, Blessed: 1, Venerable: 2 };
    return (order[a.status] || 3) - (order[b.status] || 3);
  });
  
  console.log(`Validating ${withImages.length} images (Saints first)...`);
  ...
  for (let i = 0; i < withImages.length; i++) {
    const { file, name, image, status } = withImages[i];
    const ok = await validateImage(image);
    
    if (ok) {
      valid++;
    } else {
      broken++;
      brokenFiles.push({ file, name, image, status });
      console.log(`  BROKEN [${status}]: ${name} - ${image}`);
    }
  ...
  fs.writeFileSync('C:\\iVGeek\\communion-of-saints\\scripts\\image-validation-full.json', 
